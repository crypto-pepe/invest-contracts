...
import { setContract, setMultisig } from '../../steps/sponsorship.manager';
...
export const mochaHooks = async (): Promise<Mocha.RootHookObject> => {
  return {
    async beforeAll(this: Mocha.Context) {
      ...
      // 4. init sponsorship manager
      await initSponsorshipManager(contracts);
...
async function initSponsorshipManager(contracts: Contract[]) {
  const multisig = contracts.filter(c => c.name == 'multisig')[0];
  setContract(contracts.filter(c => c.name == 'sponsorship_manager')[0]);
  await setMultisig(base58Encode(multisig.dApp));
}
...
async function initWavesAdapter(
  ...
  try {
    await initWAdapter(
      ...
      contracts.filter(c => c.name === 'leasing_node')[0].dApp,
