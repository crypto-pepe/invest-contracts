...
import { checkpoint, deposit, withdraw } from '../../steps/tokenized.staking';
import { getAssetInfo, setInt, setSignedContext, signedTransfer } from '../../steps/common';
...
describe('E2E main staking sceneries', function() {
    ...
    xit('revese calling depo + staking with varioust prev claiming', async () => {
        const node = getContractByName('leasing_node_new', this.parent?.ctx);
        ...
        const sWaves: Asset = {
            ...
            quantity: 1,
            decimals: 8,
            assetId: await getDataValue(tokenizedStake, 'ASSET', env.network)
        };
        let startRates: any = {
        ...
        for(let i = 0; i < 32; i++) {
            if(Math.random() > 0.55) {
                ...
                startRates = commitStakingRates(
                    ...
                    4,
                    reward - infraFee - 2 * env.network.invokeFee,
    ...
    it('[SANITY] staking and claim infraFee without depo first', async () => {
        ...
        await step('check balances', async () => {
            ...
            ).to.be.equal(startUserBalance - depoAmount);
            ...
            expect(
                await getBalance(adapter.dApp, env.network)
            ).to.be.equal(startAdapterBalance + depoAmount + (reward - 2 * env.network.invokeFee - infraFee) + (reward - infraFee) - env.network.invokeFee);
    ...
    it('staking and withdraw', async () => {
        ...
        const lastBlock = await getDataValue(tokenizedStake, 'LAST_HEIGHT', env.network);
        const targetBlock = await getDataValue(tokenizedStake, 'TARGET_HEIGHT', env.network);
        ...
        const wdRates = withdrawRates(
            ...
            depoRates.sw_deposit,
            {
                ...
                last_block: await getDataValue(tokenizedStake, 'LAST_HEIGHT', env.network)
            },
            assetDataQty + depoRates.sw_deposit
        );
        await step('check state', async () => {
        ...
        await step('check balances', async () => {
            // user = startBalance - depoAmount + userReward
            expect(
                await getBalance(user.address, env.network)
            ...
            ).to.be.equal(startCUBalance + env.network.invokeFee);
            // lease node = 0
            expect(
                await getBalance(node.dApp, env.network)
            ).to.be.equal(0);
            // adapter = startBalance + reward - 2x invokeFee - infraFee + deposit - withdraw
            const currAdapterBalance = await getBalance(adapter.dApp, env.network);
            expect(currAdapterBalance).to.be.equal(startAdapterBalance + reward - infraFee - 2 * env.network.invokeFee + depoAmount - wdRates.withdraw);
    ...
    it('staking and claim infraFee', async () => {
        ...
        await step('check balances', async () => {
            ...
            expect(
                await getBalance(user.address, env.network)
            ).to.be.equal(startUserBalance - 2 * depoAmount - env.network.invokeFee - env.network.transferFee);
            // manager acc = startBalance + 2 * infraFee
            expect(
                await getBalance(manager.address, env.network)
            ).to.be.equal(startManagerBalance + 2 * infraFee);
            ...
            expect(
                await getBalance(adapter.dApp, env.network)
            ).to.be.equal(startAdapterBalance + 2 * depoAmount + (reward - 2 * env.network.invokeFee - infraFee) + (reward - infraFee) - env.network.invokeFee);
            // tokenized = startBalance - invokeFee
            expect(
                await getBalance(tokenizedStake.dApp, env.network)
            ).to.be.equal(startTokenizedBalance - env.network.invokeFee);
    ...
    it('smoozy scenery with more deposits, withdraws and claim infraFee', async () => {
        ...
        let lastBlock = await getDataValue(tokenizedStake, 'LAST_HEIGHT', env.network);
        let targetBlock = await getDataValue(tokenizedStake, 'TARGET_HEIGHT', env.network);
        ...
        lastBlock = await getDataValue(tokenizedStake, 'LAST_HEIGHT', env.network);
        targetBlock = await getDataValue(tokenizedStake, 'TARGET_HEIGHT', env.network);
        ...
        await step('check balances', async () => {
            ...
            ).to.be.equal(startUser2Balance - depoAmount2 + wd2Rates.withdraw - env.network.invokeFee);
            expect(
            ...
            expect(
                await getBalance(adapter.dApp, env.network)
            ).to.be.equal(startAdapterBalance + depoAmount + depoAmount2 + depoAmount3 + 2 * reward - 2 * infraFee - 3 * env.network.invokeFee - wdRates.withdraw - wd2Rates.withdraw);
            // tokenizer
            expect(
                await getBalance(tokenizedStake.dApp, env.network)
            ).to.be.equal(startTokenizedBalance - env.network.invokeFee);
